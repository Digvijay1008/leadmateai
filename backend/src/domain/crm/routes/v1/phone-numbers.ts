...
router.post('/sip-trunks', async (req, res) => {
    try {
        const tenantId = (req as any).auth.tenantId;
        const { name, provider_name, sip_host, username, password, numbers, inbound_numbers, outbound_proxy, transport } = req.body;

        if (!sip_host) {
            return res.status(400).json({ error: 'SIP host is required' });
        }

        // Phone numbers that this trunk will handle (support both numbers and inbound_numbers key for compatibility)
        const trunkNumbers: string[] = numbers || inbound_numbers || [];
        ...
        // 3. Automatically add the numbers to phone_numbers so they appear in the UI
        for (const number of trunkNumbers) {
            await queryOne(
                `INSERT INTO phone_numbers 
                    (tenant_id, number, provider, sip_trunk_id, is_active, country_code) 
                 VALUES ($1, $2, $3, $4, true, 'IN')`,
                [tenantId, number, 'sip', result.id]
            );
        }
